...
### Automatic Recon
With the `recon` option, autoscan will automatically recommend and run the best recon tools for each found port.  
If a recommended tool is missing from your machine, autoscan will suggest how to install it.

### Runs on any shell
autoscan is 100% POSIX compatible, so it can run on any `sh` shell, and on any unix-based machine (*even a 10 YO router!*), which makes autoscan ideal for lateral movement recon.

If you want to run autoscan on a remote machine, simply download a static nmap binary from [this link](https://github.com/andrew-d/static-binaries/raw/master/binaries/linux/x86_64/nmap), or with [static-get](https://github.com/minos-org/minos-static), and transfer it to the remote machine. You can then use `-s/--static-nmap` to specify the path to the static nmap binary.

### Remote Mode (Beta)
With the `-r/--remote` flag autoscan will run in Remote Mode, which is designed to run using POSIX shell commands only, without relying on any external tools.  
...
### Output
autoscan saves the output of each type of scan is saved into a separate file, under the output directory.  
The entire script output is also saved, which you can view with `less -r outputDir/autoscan_host_type.txt`, or you can simply `cat` it.
...
## Installation:
```bash
git clone https://github.com/21y4d/autoscan.git
sudo ln -s $(pwd)/autoscan/autoscan.sh /usr/local/bin/
```
...
```
./autoscan.sh -h
Usage: autoscan.sh -H/--host <TARGET-IP> -t/--type <TYPE>
...
**Example scans**:
```
./autoscan.sh --host 10.1.1.1 --type All
./autoscan.sh -H 10.1.1.1 -t Basic
./autoscan.sh -H academy.htb -t Recon -d 1.1.1.1
./autoscan.sh -H 10.10.10.10 -t network -s ./nmap
```
...
- [x] Add network scanning type, so autoscan can discover live hosts on the network.
